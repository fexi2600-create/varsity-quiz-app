...
if not api_key:
    st.warning("দয়া করে Streamlit Secrets-এ আপনার Gemini API Key যুক্ত করুন।")
else:
...
num_questions = st.slider("প্রশ্নের সংখ্যা:", min_value=5, max_value=20, value=10)
...
if st.button("🚀 কুইজ তৈরি করো"):
    if not extracted_content and not uploaded_image:
        st.error("দয়া করে কোনো ফাইল, ছবি বা টেক্সট ইনপুট দাও!")
    else:
        with st.spinner("ঢাকা বিশ্ববিদ্যালয় ভর্তি পরীক্ষার আদলে প্রশ্ন তৈরি হচ্ছে..."):
            model = genai.GenerativeModel('gemini-1.5-flash')
            
            prompt = f"""
            তুমি ঢাকা বিশ্ববিদ্যালয় ভর্তি পরীক্ষার একজন প্রশ্ন প্রণেতা। নিচের কন্টেন্ট থেকে {num_questions} টি উচ্চমানের বহুনির্বাচনী প্রশ্ন (MCQ) তৈরি করো `{subject}` বিষয়ের জন্য। 
